...
def listOperation(lista,op): #Transforma lista do csv em lista de JSON aplicando a opereção fornecida
    if op == '' or re.search(r'[^0-9.(),]',lista): #Verifica se operação é nula ou se a lista não é só de numeros
        if re.search(r'[^0-9.(),]',lista): #Se não for só de numero transforma numa lista de strings
...
def transform(first_line,linha):
    ...
    for i in range(len(first_line)): #Percorre cada um dos elementos da linha a transformar
        if re.search(r'\*',first_line[i]): #Verifica se é ou não uma lista
            ...
            if re.search(r'^[0-9.(),]+$',linha[i]): #Verifica se parcela tem apenas numeros (para verificar se é possivel fazer operações)
